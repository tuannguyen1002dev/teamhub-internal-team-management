...
import {useFeatureAlert} from "@/contexts/FeaturesAlertProvider";
...
export default function InvitationTable() {
  const { loading, accountList } = useAccountContext();
  ...
  if (loading)
    return <div className="p-4 text-white/70">Loading accounts...</div>

  if (!accountList.length)
    return <div className="p-4 text-white/50">No accounts found</div>
...
function InvitationRow({ item, onCopy }: {
  item: AccountTableProps
  ...
  return (
    <tr className="text-sm border-t border-white/5">
      ...
      <td className="px-4 py-2 text-center border-r border-white/10">{item.role}</td>
      <td className="px-4 py-2 text-center border-r border-white/10">
        {item.createdAt ? formatDateTime(item.createdAt) : '-'}
      </td>
      <td className="px-4 py-2 text-center cursor-pointer flex justify-center items-center" >
        {item.createdBy}
      </td>
    </tr>
  )
